...
export async function upload_receipt(
  ...
  try {
    ...
    try {
      ...
      console.log(
        `MIME type check: ${mimeType === 'image/heic'} || ${mimeType === 'image/heif'}`,
      );

      if (mimeType === 'image/heic' || mimeType === 'image/heif') {
        // For HEIC files, use heic-convert first, then @napi-rs/image for WebP
        console.log(
          '✅ HEIC detected! Converting HEIC to JPEG first, then to WebP',
        );
        ...
        console.log(
          `HEIC converted to WebP successfully. Original size: ${file.length}, WebP size: ${uploadBuffer.length}`,
        );
      } else {
        // Use @napi-rs/image directly for other formats
        console.log(
          '❌ HEIC NOT detected! Using @napi-rs/image for non-HEIC formats',
        );
        console.log(`Actual MIME type: "${mimeType}"`);
        const transformer = new Transformer(file);
        uploadBuffer = Buffer.from(await transformer.webp(85)); // 85% quality
        console.log(
          `Image converted to WebP successfully. Original size: ${file.length}, WebP size: ${uploadBuffer.length}`,
        );
      }
...
export async function upload_product(
  ...
  try {
    console.log('Starting upload_product function');
    ...
    // Use appropriate extension based on whether conversion succeeded
    const filename = `product_${Date.now()}_${uuidv4()}.${fileExtension}`;
    const path = `${user_sk}/${store_sk}/${filename}`;
